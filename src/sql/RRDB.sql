...
CREATE TABLE IF NOT EXISTS `RRDB`.`Invitation` (
  `Id` INT NOT NULL AUTO_INCREMENT,
  `IDFR` VARCHAR(256) NOT NULL,
  `Name` VARCHAR(45) NULL DEFAULT NULL,
  `Lastname` VARCHAR(45) NULL DEFAULT NULL,
  ...
  `Timestamp` TIMESTAMP NOT NULL,
  PRIMARY KEY (`Id`),
  INDEX `Invitation_AuthorityUser_idx` (`AuthorityUserId` ASC) VISIBLE,
  CONSTRAINT `Invitation_AuthorityUser`
    FOREIGN KEY (`AuthorityUserId`)
    REFERENCES `RRDB`.`User` (`Id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
    ENGINE = InnoDB;
